...
class Feature_RNNLM(nn.Module):
  # the ith row of the feature table is the set of features for word i in
  # phone2ix
  def __init__(self, params, feature_table):
  ...
  def forward(self, batch):
    ...
    # the final will be batch size x seq_len x number of features
    full_representation = torch.zeros(
        batches, seq_len, num_feats, requires_grad=False)
