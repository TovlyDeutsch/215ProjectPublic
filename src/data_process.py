...
def process_data(string_training_data, dev=True, training_split=60):
  random.Random(0).shuffle(string_training_data)
  ...
  inventory = list(
      set(phone for word in string_training_data for phone in word))
...
def process_features(file_path, inventory):
  ...
  for line in file:
    ...
    if line[0] in inventory:
      feature_dict[line[0]] = [1. if x ==
                               '+' else 0. if x == '0' else -1. for x in line[1:]]
